...
void mk_allocator_heap_construct(mk_allocator_heap_t* const self)
{
	...
	heap = mk_win_kernel_heap_create(mk_win_kernel_heap_create_no_serialize | mk_win_kernel_heap_create_generate_exceptions, 0, 0);
...
void* mk_allocator_heap_allocate(mk_allocator_heap_t* const self, mk_size_t const len, mk_size_t* const real_len)
{
	...
	ret = mk_win_kernel_heap_alloc(self->m_heap, mk_win_kernel_heap_alloc_no_serialize | mk_win_kernel_heap_alloc_generate_exceptions, len);
	MK_ASSERT(ret);
...
void* mk_allocator_heap_reallocate_inplace(mk_allocator_heap_t* const self, void* const ptr, mk_size_t const len)
{
	...
	ret = mk_win_kernel_heap_realloc(self->m_heap, mk_win_kernel_heap_realloc_no_serialize | mk_win_kernel_heap_realloc_realloc_in_place_only, ptr, len);
...
void* mk_allocator_heap_reallocate_copy(mk_allocator_heap_t* const self, void* const ptr, mk_size_t const len)
{
	...
	ret = mk_win_kernel_heap_realloc(self->m_heap, mk_win_kernel_heap_realloc_no_serialize | mk_win_kernel_heap_realloc_generate_exceptions, ptr, len);
	MK_ASSERT(ret);
	return ret;
}
